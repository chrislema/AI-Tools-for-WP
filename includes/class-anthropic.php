 ...
 *
 * Implementation of AI provider for Anthropic Claude API.
 *
...
/**
 * Class AITWP_Anthropic
 *
 * Anthropic Claude API implementation.
 */
class AITWP_Anthropic extends AITWP_AI_Provider {
    ...
    public function __construct( $api_key = '' ) {
        ...
        $this->provider_id   = 'anthropic';
        $this->provider_name = 'Anthropic';
        $this->default_model = 'claude-sonnet-4-20250514';
    }
    ...
    public function analyze_for_categories( $content, $categories, $tags, $audience = array() ) {
        if ( ! $this->is_configured() ) {
            return new WP_Error( 'not_configured', __( 'Anthropic API key is not configured.', 'ai-tools-for-wp' ) );
        }

        $system_prompt = $this->build_categorization_prompt( $categories, $tags, $audience );

        $response = $this->make_request( '/messages', array(
        ...
        $message = $this->extract_text_content( $response );
    ...
    public function rewrite_content( $content, $voice_profile, $audience = array() ) {
        ...
        $rewritten = $this->extract_text_content( $response );
    ...
    protected function make_request( $endpoint, $body, $method = 'POST' ) {
        ...
        if ( $status_code >= 400 ) {
            ...
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                error_log( sprintf(
                    'AI Tools for WP: Anthropic API error (%d): %s',
                    $status_code,
                    $error_message
                ) );
            }
     ...
     * @return string
     */
    private function extract_text_content( $response ) {
